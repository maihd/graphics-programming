...
ItemFlag :: enum i32 {
	...
	LiveEditOnInputText   = 7, // true     // InputText: apply keyboard edits to backing value while typing. Otherwise, edits are applied when validating, tabbing out or losing focus.
	LiveEditOnInputScalar = 8, // false    // DragXXX, SliderXXX, InputScalar: apply keyboard edits to backing value while typing. Otherwise, edits are applied when validating, tabbing out or losing focus.
}

ITEM_FLAGS_LIVE_EDIT_ON_INPUT :: ItemFlags{.LiveEditOnInputText, .LiveEditOnInputScalar}
...
InputTextFlag :: enum i32 {
	...
	EnterReturnsTrue    = 6, // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider disabling LiveEdit! or using IsItemDeactivatedAfterEdit() instead!
...
StyleVar :: enum i32 {
	...
	MenuItemRounding            = 35, // float     MenuItemRounding
	SelectableRounding          = 36, // float     SelectableRounding
	DragDropTargetRounding      = 37, // float     DragDropTargetRounding
	ButtonTextAlign             = 38, // ImVec2    ButtonTextAlign
	SelectableTextAlign         = 39, // ImVec2    SelectableTextAlign
	SeparatorSize               = 40, // float     SeparatorSize
	SeparatorTextBorderSize     = 41, // float     SeparatorTextBorderSize
	SeparatorTextAlign          = 42, // ImVec2    SeparatorTextAlign
	SeparatorTextPadding        = 43, // ImVec2    SeparatorTextPadding
	DockingSeparatorSize        = 44, // float     DockingSeparatorSize
}
...
ColorEditFlag :: enum i32 {
	...
	PickerNoRotate   = 27, // [Picker]     // ColorPicker: disable rotating Sat/Value triangle. Best set in io.ConfigColorEditFlags once.
	InputRGB         = 28, // [Input]      // ColorEdit, ColorPicker: input and output data in RGB format.
	InputHSV         = 29, // [Input]      // ColorEdit, ColorPicker: input and output data in HSV format.
}
...
DrawFlag :: enum i32 {
	RoundCornersTopLeft     = 4, // Round top-left corner only (when rounding > 0.0f, we default to all corners).
	RoundCornersTopRight    = 5, // Round top-right corner only (when rounding > 0.0f, we default to all corners).
	RoundCornersBottomLeft  = 6, // Round bottom-left corner only (when rounding > 0.0f, we default to all corners).
	RoundCornersBottomRight = 7, // Round bottom-right corner only (when rounding > 0.0f, we default to all corners).
	RoundCornersNone        = 8, // Disable rounding even if `float rounding > 0.0f`. This is NOT zero, NOT an implicit flag!
	Closed                  = 9, // PathStroke(), AddPolyline(): specify that shape should be closed.
}
...
DrawListFlag :: enum i32 {
	...
	TextNoPixelSnap        = 4, // Disable automatically snapping AddText() calls to pixel boundaries.
}
...
TableColumnSortSpecs :: struct {
	ColumnUserID:  ID, // User data for the column (if specified by a TableSetupColumn() call in the 'ImGuiID user_data' field). FIXME: Should be called 'UserData'..
...
Style :: struct {
	...
	MenuItemRounding:                 f32, // Radius of MenuItem, BeginMenu rounding.
	SelectableRounding:               f32, // Radius of Selectable rounding. MODIFYING THIS IS DISCOURAGED. CONTIGUOUS SELECTIONS WILL NOT LOOK RIGHT. (#7589)
	...
	InputTextCursorSize:              f32, // Thickness of cursor/caret in InputText().
...
IO :: struct {
	...
	ConfigIniSettingsSaveLastUsedDate:             bool, // = true         // Enable loading/saving last used day (YYYYMMDD) in some .ini struct, making things easier to audit and allowing custom tools to cleanup old data.
	ConfigIniSettingsAutoDiscardMonths:            i32, // = 0          // [BETA] Set number of months after which unused .ini entries are discarded on load. Require platform_io.Platform_SessionDate to be set. For systems supporting the feature, .ini entries without a LastUsed field will always be discarded! Please report if you are using this.
	...
	MouseDoubleClickTime:                          f32, // = 0.30f          // Time for consecutive clicks to account as a double-click, in seconds.
	MouseDoubleClickMaxDist:                       f32, // = 6.0f           // Distance threshold to stay in to validate a double-click or multiple clicks, in pixels.
	MouseSingleClickDelay:                         f32, // = 0.60f          // Time for a delayed click when using GetItemClickedCountWithSingleClickDelay() or IsMouseReleasedWithDelay(), in seconds. Must be > io.MouseDoubleClickTime.
...
DrawData :: struct {
	...
	FrameCount:       i32, // Frame counter of the emitter context. Mostly for debugging purpose.
...
TextureRect :: struct {
	x: u16, // Upper-left coordinates of rectangle to update, within the parent Pixels[] array.
	y: u16, // Upper-left coordinates of rectangle to update, within the parent Pixels[] array.
...
TextureData :: struct {
	...
	QueueUserData:        rawptr, // r    -   // Convenience storage for a staged/multi-threaded rendering texture queue (e.g. imgui_threaded_rendering.h. See #8597). When != NULL, core assumes the texture is referenced by the queue.
	...
	Pixels:               ^u8, // w    r   // Pointer to whole texture buffer holding 'Width*Height' pixels and 'Width*Height*BytesPerPixels' bytes.
...
PlatformIO :: struct {
	...
	Platform_SessionDate:              i32, // Integer storing YYYYMMDD e.g. 20261231 corresponding to the beginning of application session.
	...
	PlatformCreateVkSurface:           proc "c" (
		vp: ^Viewport,
		vk_inst: u64,
...
foreign imguilib {
	...
	TextureRef_GetTexID :: proc(self: ^TextureRef) -> TextureID ---
	...
	BeginChild :: proc(str_id: cstring, size: Vec2 = Vec2{0, 0}, child_flags: ChildFlags = {}, window_flags: WindowFlags = {}) -> bool ---
	@(link_name = "ImGui_BeginChildID")
	BeginChildID :: proc(id: ID, size: Vec2 = Vec2{0, 0}, child_flags: ChildFlags = {}, window_flags: WindowFlags = {}) -> bool ---
	...
	IsWindowHovered :: proc(flags: HoveredFlags = {}) -> bool ---
	...
	SetNextWindowPos :: proc(pos: Vec2, cond: Cond = {}, pivot: Vec2 = Vec2{0, 0}) ---
	// set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
	@(link_name = "ImGui_SetNextWindowSize")
	SetNextWindowSize :: proc(size: Vec2, cond: Cond = {}) ---
	// set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
	@(link_name = "ImGui_SetNextWindowSizeConstraints")
	SetNextWindowSizeConstraints :: proc(size_min: Vec2, size_max: Vec2, custom_callback: SizeCallback = nil, custom_callback_data: rawptr = nil) ---
	// set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
	@(link_name = "ImGui_SetNextWindowContentSize")
	SetNextWindowContentSize :: proc(size: Vec2) ---
	...
	SetNextWindowScroll :: proc(scroll: Vec2) ---
	...
	SetWindowPos :: proc(pos: Vec2, cond: Cond = {}) ---
	// (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
	@(link_name = "ImGui_SetWindowSize")
	SetWindowSize :: proc(size: Vec2, cond: Cond = {}) ---
	// (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
	@(link_name = "ImGui_SetWindowCollapsed")
	SetWindowCollapsed :: proc(collapsed: bool, cond: Cond = {}) ---
	// (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
	@(link_name = "ImGui_SetWindowFocus")
	SetWindowFocus :: proc() ---
	// set named window position.
	@(link_name = "ImGui_SetWindowPosStr")
	SetWindowPosStr :: proc(name: cstring, pos: Vec2, cond: Cond = {}) ---
	// set named window size. set axis to 0.0f to force an auto-fit on this axis.
	@(link_name = "ImGui_SetWindowSizeStr")
	SetWindowSizeStr :: proc(name: cstring, size: Vec2, cond: Cond = {}) ---
	// set named window collapsed state
	@(link_name = "ImGui_SetWindowCollapsedStr")
	SetWindowCollapsedStr :: proc(name: cstring, collapsed: bool, cond: Cond = {}) ---
	...
	SetScrollFromPosX :: proc(local_x: f32, center_x_ratio: f32 = 0.5) ---
	// adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
	@(link_name = "ImGui_SetScrollFromPosY")
	SetScrollFromPosY :: proc(local_y: f32, center_y_ratio: f32 = 0.5) ---
	...
	PushFontFloat :: proc(font: ^Font, font_size_base_unscaled: f32) ---
	...
	PushStyleColor :: proc(idx: Col, col: u32) ---
	@(link_name = "ImGui_PushStyleColorImVec4")
	PushStyleColorVec4 :: proc(idx: Col, col: Vec4) ---
	...
	PushStyleVarVec2 :: proc(idx: StyleVar, val: Vec2) ---
	...
	PushItemFlag :: proc(option: ItemFlags, enabled: bool) ---
	...
	GetColorU32 :: proc(idx: Col, alpha_mul: f32 = 1.0) -> u32 ---
	// retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
	@(link_name = "ImGui_GetColorU32ImVec4")
	GetColorU32Vec4 :: proc(col: Vec4) -> u32 ---
	// retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
	@(link_name = "ImGui_GetColorU32ImU32")
	GetColorU32U32 :: proc(col: u32, alpha_mul: f32 = 1.0) -> u32 ---
	// retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
	@(link_name = "ImGui_GetStyleColorVec4")
	GetStyleColorVec4 :: proc(idx: Col) -> ^Vec4 ---
	...
	SetCursorScreenPos :: proc(pos: Vec2) ---
	...
	SetCursorPos :: proc(local_pos: Vec2) ---
	// [window-local] "
	...
	TextColored :: proc(col: Vec4, fmt: cstring, #c_vararg args: ..any) ---
	// shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
	@(link_name = "ImGui_TextDisabled")
	TextDisabled :: proc(fmt: cstring, #c_vararg args: ..any) ---
	// shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	@(link_name = "ImGui_TextWrapped")
	TextWrapped :: proc(fmt: cstring, #c_vararg args: ..any) ---
	// display text+label aligned the same way as value+label widgets
	@(link_name = "ImGui_LabelText")
	LabelText :: proc(label: cstring, fmt: cstring, #c_vararg args: ..any) ---
	// shortcut for Bullet()+Text()
	@(link_name = "ImGui_BulletText")
	BulletText :: proc(fmt: cstring, #c_vararg args: ..any) ---
	...
	InvisibleButton :: proc(str_id: cstring, size: Vec2, flags: ButtonFlags = {}) -> bool ---
	...
	CheckboxFlagsIntPtr :: proc(label: cstring, flags: ^i32, flags_value: i32) -> bool ---
	@(link_name = "ImGui_CheckboxFlagsUintPtr")
	CheckboxFlagsUintPtr :: proc(label: cstring, flags: ^u32, flags_value: u32) -> bool ---
	// use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
	@(link_name = "ImGui_RadioButton")
	RadioButton :: proc(label: cstring, active: bool) -> bool ---
	// shortcut to handle the above pattern when value is an integer
	@(link_name = "ImGui_RadioButtonIntPtr")
	RadioButtonIntPtr :: proc(label: cstring, v: ^i32, v_button: i32) -> bool ---
	@(link_name = "ImGui_ProgressBar")
	ProgressBar :: proc(fraction: f32, size_arg: Vec2 = Vec2{-min(f32), 0}, overlay: cstring = nil) ---
	// draw a small circle + keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
	@(link_name = "ImGui_Bullet")
	Bullet :: proc() ---
	// hyperlink text button, return true when clicked
	@(link_name = "ImGui_TextLink")
	TextLink :: proc(label: cstring) -> bool ---
	// hyperlink text button, automatically open file/url when clicked
	@(link_name = "ImGui_TextLinkOpenURL")
	TextLinkOpenURL :: proc(label: cstring, url: cstring = nil) -> bool ---
	// Widgets: Images
	...
	Image :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}) ---
	@(link_name = "ImGui_ImageWithBg")
	ImageWithBg :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) ---
	@(link_name = "ImGui_ImageButton")
	ImageButton :: proc(str_id: cstring, tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) -> bool ---
	...
	ComboChar :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
	...
	ComboCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
	...
	DragFloat :: proc(label: cstring, v: ^f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", flags: SliderFlags = {}) -> bool ---
	...
	DragInt :: proc(label: cstring, v: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_DragInt2")
	DragInt2 :: proc(label: cstring, v: ^[2]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_DragInt3")
	DragInt3 :: proc(label: cstring, v: ^[3]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_DragInt4")
	DragInt4 :: proc(label: cstring, v: ^[4]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_DragIntRange2")
	DragIntRange2 :: proc(label: cstring, v_current_min: ^i32, v_current_max: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", format_max: cstring = nil, flags: SliderFlags = {}) -> bool ---
	...
	DragScalarN :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, v_speed: f32 = 1.0, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: SliderFlags = {}) -> bool ---
	...
	SliderFloat :: proc(label: cstring, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: SliderFlags = {}) -> bool ---
	...
	SliderInt :: proc(label: cstring, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_SliderInt2")
	SliderInt2 :: proc(label: cstring, v: ^[2]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_SliderInt3")
	SliderInt3 :: proc(label: cstring, v: ^[3]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	@(link_name = "ImGui_SliderInt4")
	SliderInt4 :: proc(label: cstring, v: ^[4]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	...
	SliderScalarN :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: SliderFlags = {}) -> bool ---
	...
	VSliderInt :: proc(label: cstring, size: Vec2, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool ---
	...
	InputText :: proc(label: cstring, buf: cstring, buf_size: uint, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	@(link_name = "ImGui_InputTextMultiline")
	InputTextMultiline :: proc(label: cstring, buf: cstring, buf_size: uint, size: Vec2 = Vec2{0, 0}, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	@(link_name = "ImGui_InputTextWithHint")
	InputTextWithHint :: proc(label: cstring, hint: cstring, buf: cstring, buf_size: uint, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	...
	InputInt :: proc(label: cstring, v: ^i32, step: i32 = 1, step_fast: i32 = 100, flags: InputTextFlags = {}) -> bool ---
	@(link_name = "ImGui_InputInt2")
	InputInt2 :: proc(label: cstring, v: ^[2]i32, flags: InputTextFlags = {}) -> bool ---
	@(link_name = "ImGui_InputInt3")
	InputInt3 :: proc(label: cstring, v: ^[3]i32, flags: InputTextFlags = {}) -> bool ---
	@(link_name = "ImGui_InputInt4")
	InputInt4 :: proc(label: cstring, v: ^[4]i32, flags: InputTextFlags = {}) -> bool ---
	...
	InputScalarN :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: InputTextFlags = {}) -> bool ---
	...
	ColorButton :: proc(desc_id: cstring, col: Vec4, flags: ColorEditFlags = {}, size: Vec2 = Vec2{0, 0}) -> bool ---
	// Widgets: Trees
	...
	TreeNodeStr :: proc(str_id: cstring, fmt: cstring, #c_vararg args: ..any) -> bool ---
	// "
	...
	CollapsingHeader :: proc(label: cstring, flags: TreeNodeFlags = {}) -> bool ---
	// when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
	@(link_name = "ImGui_CollapsingHeaderBoolPtr")
	CollapsingHeaderBoolPtr :: proc(label: cstring, p_visible: ^bool, flags: TreeNodeFlags = {}) -> bool ---
	...
	SelectableBoolPtr :: proc(label: cstring, p_selected: ^bool, flags: SelectableFlags = {}, size: Vec2 = Vec2{0, 0}) -> bool ---
	...
	BeginMultiSelect :: proc(flags: MultiSelectFlags, selection_size: i32 = -1, items_count: i32 = -1) -> ^MultiSelectIO ---
	...
	BeginListBox :: proc(label: cstring, size: Vec2 = Vec2{0, 0}) -> bool ---
	// only call EndListBox() if BeginListBox() returned true!
	@(link_name = "ImGui_EndListBox")
	EndListBox :: proc() ---
	@(link_name = "ImGui_ListBox")
	ListBox :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, height_in_items: i32 = -1) -> bool ---
	@(link_name = "ImGui_ListBoxCallback")
	ListBoxCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, height_in_items: i32 = -1) -> bool ---
	// Widgets: Data Plotting
	// - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
	@(link_name = "ImGui_PlotLines")
	PlotLines :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name = "ImGui_PlotLinesCallback")
	PlotLinesCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}) ---
	@(link_name = "ImGui_PlotHistogram")
	PlotHistogram :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name = "ImGui_PlotHistogramCallback")
	PlotHistogramCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}) ---
	// Widgets: Menus
	...
	MenuItem :: proc(label: cstring, shortcut: cstring = nil, selected: bool = false, enabled: bool = true) -> bool ---
	// return true when activated + toggle (*p_selected) if p_selected != NULL
	@(link_name = "ImGui_MenuItemBoolPtr")
	MenuItemBoolPtr :: proc(label: cstring, shortcut: cstring, p_selected: ^bool, enabled: bool = true) -> bool ---
	// Tooltips
	...
	SetTooltip :: proc(fmt: cstring, #c_vararg args: ..any) ---
	...
	SetItemTooltip :: proc(fmt: cstring, #c_vararg args: ..any) ---
	// Popups, Modals
	...
	BeginPopup :: proc(str_id: cstring, flags: WindowFlags = {}) -> bool ---
	// return true if the modal is open, and you can start outputting to it.
	@(link_name = "ImGui_BeginPopupModal")
	BeginPopupModal :: proc(name: cstring, p_open: ^bool = nil, flags: WindowFlags = {}) -> bool ---
	...
	OpenPopup :: proc(str_id: cstring, popup_flags: PopupFlags = {}) -> bool ---
	// id overload to facilitate calling from nested stacks
	@(link_name = "ImGui_OpenPopupID")
	OpenPopupID :: proc(id: ID, popup_flags: PopupFlags = {}) -> bool ---
	// helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
	@(link_name = "ImGui_OpenPopupOnItemClick")
	OpenPopupOnItemClick :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool ---
	...
	BeginPopupContextItem :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool ---
	// open+begin popup when clicked on current window.
	@(link_name = "ImGui_BeginPopupContextWindow")
	BeginPopupContextWindow :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool ---
	// open+begin popup when clicked in void (where there are no windows).
	@(link_name = "ImGui_BeginPopupContextVoid")
	BeginPopupContextVoid :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool ---
	...
	IsPopupOpen :: proc(str_id: cstring, flags: PopupFlags = {}) -> bool ---
	// Tables
	...
	BeginTable :: proc(str_id: cstring, columns: i32, flags: TableFlags = {}, outer_size: Vec2 = Vec2{0.0, 0.0}, inner_width: f32 = 0.0) -> bool ---
	...
	TableNextRow :: proc(row_flags: TableRowFlags = {}, min_row_height: f32 = 0.0) ---
	...
	TableSetupColumn :: proc(label: cstring, flags: TableColumnFlags = {}, init_width_or_weight: f32 = 0.0, user_data: ID = {}) ---
	...
	TableGetColumnName :: proc(column_n: i32 = -1) -> cstring ---
	// return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
	@(link_name = "ImGui_TableGetColumnFlags")
	TableGetColumnFlags :: proc(column_n: i32 = -1) -> TableColumnFlags ---
	...
	TableSetBgColor :: proc(target: TableBgTarget, color: u32, column_n: i32 = -1) ---
	...
	BeginTabBar :: proc(str_id: cstring, flags: TabBarFlags = {}) -> bool ---
	// only call EndTabBar() if BeginTabBar() returns true!
	@(link_name = "ImGui_EndTabBar")
	EndTabBar :: proc() ---
	// create a Tab. Returns true if the Tab is selected.
	@(link_name = "ImGui_BeginTabItem")
	BeginTabItem :: proc(label: cstring, p_open: ^bool = nil, flags: TabItemFlags = {}) -> bool ---
	// only call EndTabItem() if BeginTabItem() returns true!
	@(link_name = "ImGui_EndTabItem")
	EndTabItem :: proc() ---
	// create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
	@(link_name = "ImGui_TabItemButton")
	TabItemButton :: proc(label: cstring, flags: TabItemFlags = {}) -> bool ---
	...
	DockSpace :: proc(dockspace_id: ID, size: Vec2 = Vec2{0, 0}, flags: DockNodeFlags = {}, window_class: ^WindowClass = nil) -> ID ---
	...
	SetNextWindowClass :: proc(window_class: ^WindowClass) ---
	...
	LogText :: proc(fmt: cstring, #c_vararg args: ..any) ---
	// Drag and Drop
	...
	BeginDragDropSource :: proc(flags: DragDropFlags = {}) -> bool ---
	// type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	@(link_name = "ImGui_SetDragDropPayload")
	SetDragDropPayload :: proc(type: cstring, data: rawptr, sz: uint, cond: Cond = {}) -> bool ---
	...
	IsItemHovered :: proc(flags: HoveredFlags = {}) -> bool ---
	...
	IsItemClicked :: proc(mouse_button: MouseButton = {}) -> bool ---
	...
	@(link_name = "ImGui_GetItemClickedCountWithSingleClickDelay")
	GetItemClickedCountWithSingleClickDelay :: proc(mouse_button: MouseButton = {}, delay: f32 = -1.0) -> i32 ---
	// Viewports
	// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
	...
	GetBackgroundDrawList :: proc(viewport: ^Viewport = nil) -> ^DrawList ---
	...
	GetForegroundDrawList :: proc(viewport: ^Viewport = nil) -> ^DrawList ---
	// Miscellaneous Utilities
	// test if rectangle (of given size, starting from cursor position) is visible / not clipped.
	@(link_name = "ImGui_IsRectVisibleBySize")
	IsRectVisibleBySize :: proc(size: Vec2) -> bool ---
	// test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
	@(link_name = "ImGui_IsRectVisible")
	IsRectVisible :: proc(rect_min: Vec2, rect_max: Vec2) -> bool ---
	...
	IsKeyPressed :: proc(key: Key, repeat: bool = true) -> bool ---
	...
	IsKeyChordPressed :: proc(key_chord: KeyChord) -> bool ---
	...
	SetItemKeyOwner :: proc(key: Key) -> bool ---
	...
	IsMouseClicked :: proc(button: MouseButton, repeat: bool = false) -> bool ---
	...
	IsMouseDoubleClicked :: proc(button: MouseButton) -> bool ---
	...
	IsMouseReleasedWithDelay :: proc(button: MouseButton, delay: f32 = -1.) -> bool ---
	...
	IsMouseHoveringRect :: proc(r_min: Vec2, r_max: Vec2, clip: bool = true) -> bool ---
	// by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
	@(link_name = "ImGui_IsMousePosValid")
	IsMousePosValid :: proc(mouse_pos: ^Vec2 = nil) -> bool ---
	...
	IsMouseDragging :: proc(button: MouseButton, lock_threshold: f32 = -1.0) -> bool ---
	// return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
	@(link_name = "ImGui_GetMouseDragDelta")
	GetMouseDragDelta :: proc(button: MouseButton = {}, lock_threshold: f32 = -1.0) -> Vec2 ---
	//
	...
	LoadIniSettingsFromMemory :: proc(ini_data: cstring, ini_size: uint = {}) ---
	...
	SaveIniSettingsToMemory :: proc(out_ini_size: ^uint = nil) -> cstring ---
	// Debug Utilities
	...
	DebugCheckVersionAndDataLayout :: proc(version_str: cstring, sz_io: uint, sz_style: uint, sz_vec2: uint, sz_vec4: uint, sz_drawvert: uint, sz_drawidx: uint) -> bool ---
	// Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
	@(link_name = "ImGui_DebugLog")
	DebugLog :: proc(fmt: cstring, #c_vararg args: ..any) ---
	// Memory Allocators
	...
	FindViewportByID :: proc(viewport_id: ID) -> ^Viewport ---
	...
	PlatformIO_SetPlatform_GetWindowWorkAreaInsets :: proc(getWindowWorkAreaInsetsFunc: proc "c" (vp: ^Viewport, result: ^Vec4)) ---
	...
	PlatformIO_SetPlatform_GetWindowFramebufferScale :: proc(getWindowFramebufferScaleFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
	// Set ImGuiPlatformIO::Platform_GetWindowPos in a C-compatible mannner
	@(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowPos")
	PlatformIO_SetPlatform_GetWindowPos :: proc(getWindowPosFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
	// Set ImGuiPlatformIO::Platform_GetWindowSize in a C-compatible mannner
	@(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowSize")
	PlatformIO_SetPlatform_GetWindowSize :: proc(getWindowSizeFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
	...
	IO_AddMouseSourceEvent :: proc(self: ^IO, source: MouseSource) ---
	...
	IO_AddInputCharacterUTF16 :: proc(self: ^IO, c: Wchar16) ---
	...
	IO_ClearInputMouse :: proc(self: ^IO) ---
	@(link_name = "ImGuiInputTextCallbackData_DeleteChars")
	InputTextCallbackData_DeleteChars :: proc(self: ^InputTextCallbackData, pos: i32, bytes_count: i32) ---
	@(link_name = "ImGuiInputTextCallbackData_InsertChars")
	InputTextCallbackData_InsertChars :: proc(self: ^InputTextCallbackData, pos: i32, text: cstring, text_end: cstring = nil) ---
	...
	InputTextCallbackData_SetSelection :: proc(self: ^InputTextCallbackData, s: i32, e: i32) ---
	...
	TextFilter_TextRange_empty :: proc(self: ^TextFilter_TextRange) -> bool ---
	@(link_name = "ImGuiTextFilter_ImGuiTextRange_split")
	TextFilter_TextRange_split :: proc(self: ^TextFilter_TextRange, separator: u8, out: ^Vector_TextRange) ---
	// Helper calling InputText+Build
	@(link_name = "ImGuiTextFilter_Draw")
	TextFilter_Draw :: proc(self: ^TextFilter, label: cstring = "Filter (inc,-exc)", width: f32 = 0.0) -> bool ---
	...
	TextBuffer_end :: proc(self: ^TextBuffer) -> cstring ---
	@(link_name = "ImGuiTextBuffer_size")
	TextBuffer_size :: proc(self: ^TextBuffer) -> i32 ---
	...
	TextBuffer_resize :: proc(self: ^TextBuffer, size: i32) ---
	@(link_name = "ImGuiTextBuffer_reserve")
	TextBuffer_reserve :: proc(self: ^TextBuffer, capacity: i32) ---
	...
	ListClipper_Begin :: proc(self: ^ListClipper, items_count: i32, items_height: f32 = -1.0) ---
	// Automatically called on the last call of Step() that returns false.
	@(link_name = "ImGuiListClipper_End")
	ListClipper_End :: proc(self: ^ListClipper) ---
	// Call until it returns false. The DisplayStart/DisplayEnd fields will be set and you can process/draw those items.
	@(link_name = "ImGuiListClipper_Step")
	ListClipper_Step :: proc(self: ^ListClipper) -> bool ---
	// Call IncludeItemByIndex() or IncludeItemsByIndex() *BEFORE* first call to Step() if you need a range of items to not be clipped, regardless of their visibility.
	// (Due to alignment / padding of certain items it is possible that an extra item may be included on either end of the display range).
	@(link_name = "ImGuiListClipper_IncludeItemByIndex")
	ListClipper_IncludeItemByIndex :: proc(self: ^ListClipper, item_index: i32) ---
	// item_end is exclusive e.g. use (42, 42+1) to make item 42 never clipped.
	@(link_name = "ImGuiListClipper_IncludeItemsByIndex")
	ListClipper_IncludeItemsByIndex :: proc(self: ^ListClipper, item_begin: i32, item_end: i32) ---
	// Seek cursor toward given item. This is automatically called while stepping.
	// - The only reason to call this is: you can use ImGuiListClipper::Begin(INT_MAX) if you don't know item count ahead of time.
	// - In this case, after all steps are done, you'll want to call SeekCursorForItem(item_count).
	@(link_name = "ImGuiListClipper_SeekCursorForItem")
	ListClipper_SeekCursorForItem :: proc(self: ^ListClipper, item_index: i32) ---
	...
	SelectionBasicStorage_ApplyRequests :: proc(self: ^SelectionBasicStorage, ms_io: ^MultiSelectIO) ---
	// Query if an item id is in selection.
	@(link_name = "ImGuiSelectionBasicStorage_Contains")
	SelectionBasicStorage_Contains :: proc(self: ^SelectionBasicStorage, id: ID) -> bool ---
	// Clear selection
	@(link_name = "ImGuiSelectionBasicStorage_Clear")
	SelectionBasicStorage_Clear :: proc(self: ^SelectionBasicStorage) ---
	// Swap two selections
	@(link_name = "ImGuiSelectionBasicStorage_Swap")
	SelectionBasicStorage_Swap :: proc(self: ^SelectionBasicStorage, r: ^SelectionBasicStorage) ---
	...
	SelectionBasicStorage_SetItemSelected :: proc(self: ^SelectionBasicStorage, id: ID, selected: bool) ---
	// Iterate selection with 'void* it = NULL; ImGuiID id; while (selection.GetNextSelectedItem(&it, &id)) { ... }'
	@(link_name = "ImGuiSelectionBasicStorage_GetNextSelectedItem")
	SelectionBasicStorage_GetNextSelectedItem :: proc(self: ^SelectionBasicStorage, opaque_it: ^rawptr, out_id: ^ID) -> bool ---
	// Convert index to item id based on provided adapter.
	@(link_name = "ImGuiSelectionBasicStorage_GetStorageIdFromIndex")
	SelectionBasicStorage_GetStorageIdFromIndex :: proc(self: ^SelectionBasicStorage, idx: i32) -> ID ---
	// Apply selection requests by using AdapterSetItemSelected() calls
	@(link_name = "ImGuiSelectionExternalStorage_ApplyRequests")
	SelectionExternalStorage_ApplyRequests :: proc(self: ^SelectionExternalStorage, ms_io: ^MultiSelectIO) ---
	...
	DrawListSplitter_Clear :: proc(self: ^DrawListSplitter) ---
	...
	DrawList_PushClipRect :: proc(self: ^DrawList, clip_rect_min: Vec2, clip_rect_max: Vec2, intersect_with_current_clip_rect: bool = false) ---
	...
	DrawList_AddLineH :: proc(self: ^DrawList, min_x: f32, max_x: f32, y: f32, col: u32, thickness: f32 = 1.0) ---
	@(link_name = "ImDrawList_AddLineV")
	DrawList_AddLineV :: proc(self: ^DrawList, x: f32, min_y: f32, max_y: f32, col: u32, thickness: f32 = 1.0) ---
	// a: upper-left, b: lower-right (== upper-left + size)
	@(link_name = "ImDrawList_AddRect")
	DrawList_AddRect :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, thickness: f32 = 1.0, flags: DrawFlags = {}) ---
	// a: upper-left, b: lower-right (== upper-left + size)
	@(link_name = "ImDrawList_AddRectFilled")
	DrawList_AddRectFilled :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, flags: DrawFlags = {}) ---
	...
	DrawList_AddCircle :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0, thickness: f32 = 1.0) ---
	...
	DrawList_AddNgon :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32, thickness: f32 = 1.0) ---
	...
	DrawList_AddEllipse :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0, thickness: f32 = 1.0) ---
	@(link_name = "ImDrawList_AddEllipseFilled")
	DrawList_AddEllipseFilled :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0) ---
	...
	DrawList_AddTextFontPtr :: proc(self: ^DrawList, font: ^Font, font_size: f32, pos: Vec2, col: u32, text_begin: cstring, text_end: cstring = nil, wrap_width: f32 = 0.0, cpu_fine_clip_rect: ^Vec4 = nil) ---
	// Cubic Bezier (4 control points)
	@(link_name = "ImDrawList_AddBezierCubic")
	DrawList_AddBezierCubic :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32, thickness: f32, num_segments: i32 = 0) ---
	// Quadratic Bezier (3 control points)
	@(link_name = "ImDrawList_AddBezierQuadratic")
	DrawList_AddBezierQuadratic :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, col: u32, thickness: f32, num_segments: i32 = 0) ---
	// General polygon
	// - Only simple polygons are supported by filling functions (no self-intersections, no holes).
	// - Concave polygon fill is more expensive than convex one: it has O(N^2) complexity. Provided as a convenience for the user but not used by the main library.
	@(link_name = "ImDrawList_AddPolyline")
	DrawList_AddPolyline :: proc(self: ^DrawList, points: ^Vec2, num_points: i32, col: u32, thickness: f32, flags: DrawFlags = {}) ---
	...
	ImDrawList_AddImage :: proc(self: ^DrawList, tex_ref: TextureRef, p_min: Vec2, p_max: Vec2, uv_min: Vec2 = Vec2{0, 0}, uv_max: Vec2 = Vec2{1, 1}, col: u32 = 0xff_ff_ff_ff) ---
	@(link_name = "ImDrawList_AddImageQuad")
	ImDrawList_AddImageQuad :: proc(self: ^DrawList, tex_ref: TextureRef, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1: Vec2 = Vec2{0, 0}, uv2: Vec2 = Vec2{1, 0}, uv3: Vec2 = Vec2{1, 1}, uv4: Vec2 = Vec2{0, 1}, col: u32 = 0xff_ff_ff_ff) ---
	@(link_name = "ImDrawList_AddImageRounded")
	ImDrawList_AddImageRounded :: proc(self: ^DrawList, tex_ref: TextureRef, p_min: Vec2, p_max: Vec2, uv_min: Vec2, uv_max: Vec2, col: u32, rounding: f32, flags: DrawFlags = {}) ---
	...
	DrawList_PathStroke :: proc(self: ^DrawList, col: u32, thickness: f32 = 1.0, flags: DrawFlags = {}) ---
	...
	DrawList_PathArcToFast :: proc(self: ^DrawList, center: Vec2, radius: f32, a_min_of_12: i32, a_max_of_12: i32) ---
	// Ellipse
	@(link_name = "ImDrawList_PathEllipticalArcTo")
	DrawList_PathEllipticalArcTo :: proc(self: ^DrawList, center: Vec2, radius: Vec2, rot: f32, a_min: f32, a_max: f32, num_segments: i32 = 0) ---
	// Cubic Bezier (4 control points)
	@(link_name = "ImDrawList_PathBezierCubicCurveTo")
	DrawList_PathBezierCubicCurveTo :: proc(self: ^DrawList, p2: Vec2, p3: Vec2, p4: Vec2, num_segments: i32 = 0) ---
	// Quadratic Bezier (3 control points)
	@(link_name = "ImDrawList_PathBezierQuadraticCurveTo")
	DrawList_PathBezierQuadraticCurveTo :: proc(self: ^DrawList, p2: Vec2, p3: Vec2, num_segments: i32 = 0) ---
	...
	DrawList_AddCallback :: proc(self: ^DrawList, callback: DrawCallback, userdata: rawptr = nil, userdata_size: uint = {}) ---
	// Advanced: Miscellaneous
	// This is useful if you need to forcefully create a new draw call (to allow for dependent rendering / blending). Otherwise primitives are merged into the same draw-call as much as possible
	@(link_name = "ImDrawList_AddDrawCmd")
	DrawList_AddDrawCmd :: proc(self: ^DrawList) ---
	...
	DrawList_CloneOutput :: proc(self: ^DrawList) -> ^DrawList ---
	// Advanced: Channels
	...
	DrawList_PrimRect :: proc(self: ^DrawList, a: Vec2, b: Vec2, col: u32) ---
	...
	DrawList_PrimQuadUV :: proc(self: ^DrawList, a: Vec2, b: Vec2, c: Vec2, d: Vec2, uv_a: Vec2, uv_b: Vec2, uv_c: Vec2, uv_d: Vec2, col: u32) ---
	...
	DrawList_PrimVtx :: proc(self: ^DrawList, pos: Vec2, uv: Vec2, col: u32) ---
	// [Internal helpers]
	...
	DrawData_AddDrawList :: proc(self: ^DrawData, draw_list: ^DrawList) ---
	// Helper to convert all buffers from indexed to non-indexed, in case you cannot render indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed rendering!
	@(link_name = "ImDrawData_DeIndexAllBuffers")
	DrawData_DeIndexAllBuffers :: proc(self: ^DrawData) ---
	// Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
	@(link_name = "ImDrawData_ScaleClipRects")
	DrawData_ScaleClipRects :: proc(self: ^DrawData, fb_scale: Vec2) ---
	...
	FontGlyphRangesBuilder_GetBit :: proc(self: ^FontGlyphRangesBuilder, n: uint) -> bool ---
	// Set bit n in the array
	@(link_name = "ImFontGlyphRangesBuilder_SetBit")
	FontGlyphRangesBuilder_SetBit :: proc(self: ^FontGlyphRangesBuilder, n: uint) ---
	// Add character
	@(link_name = "ImFontGlyphRangesBuilder_AddChar")
	FontGlyphRangesBuilder_AddChar :: proc(self: ^FontGlyphRangesBuilder, c: Wchar) ---
	// Add string (each character of the UTF-8 string are added)
	@(link_name = "ImFontGlyphRangesBuilder_AddText")
	FontGlyphRangesBuilder_AddText :: proc(self: ^FontGlyphRangesBuilder, text: cstring, text_end: cstring = nil) ---
	// Add ranges, e.g. builder.AddRanges(ImFontAtlas::GetGlyphRangesDefault()) to force add all of ASCII/Latin+Ext
	@(link_name = "ImFontGlyphRangesBuilder_AddRanges")
	FontGlyphRangesBuilder_AddRanges :: proc(self: ^FontGlyphRangesBuilder, ranges: ^Wchar) ---
	...
	FontGlyphRangesBuilder_BuildRanges :: proc(self: ^FontGlyphRangesBuilder, out_ranges: ^Vector_Wchar) ---
	...
	FontAtlas_AddFontDefaultVector :: proc(self: ^FontAtlas, font_cfg: ^FontConfig = nil) -> ^Font ---
	// Embedded classic pixel-clean font. Recommended at Size 13px with no scaling.
	@(link_name = "ImFontAtlas_AddFontDefaultBitmap")
	FontAtlas_AddFontDefaultBitmap :: proc(self: ^FontAtlas, font_cfg: ^FontConfig = nil) -> ^Font ---
	@(link_name = "ImFontAtlas_AddFontFromFileTTF")
	FontAtlas_AddFontFromFileTTF :: proc(self: ^FontAtlas, filename: cstring, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
	// Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be freed.
	@(link_name = "ImFontAtlas_AddFontFromMemoryTTF")
	FontAtlas_AddFontFromMemoryTTF :: proc(self: ^FontAtlas, font_data: rawptr, font_data_size: i32, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
	// 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
	@(link_name = "ImFontAtlas_AddFontFromMemoryCompressedTTF")
	FontAtlas_AddFontFromMemoryCompressedTTF :: proc(self: ^FontAtlas, compressed_font_data: rawptr, compressed_font_data_size: i32, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
	// 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
	@(link_name = "ImFontAtlas_AddFontFromMemoryCompressedBase85TTF")
	FontAtlas_AddFontFromMemoryCompressedBase85TTF :: proc(self: ^FontAtlas, compressed_font_data_base85: cstring, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
	// Remove a font
	...
	FontAtlas_CompactCache :: proc(self: ^FontAtlas) ---
	// Change font loader at runtime.
	@(link_name = "ImFontAtlas_SetFontLoader")
	FontAtlas_SetFontLoader :: proc(self: ^FontAtlas, font_loader: ^FontLoader) ---
	...
	FontAtlas_ClearInputData :: proc(self: ^FontAtlas) ---
	...
	FontAtlas_ClearTexData :: proc(self: ^FontAtlas) ---
	...
	FontAtlas_GetGlyphRangesDefault :: proc(self: ^FontAtlas) -> ^Wchar ---
	...
	FontAtlas_AddCustomRect :: proc(self: ^FontAtlas, width: i32, height: i32, out_r: ^FontAtlasRect = nil) -> FontAtlasRectId ---
	...
	FontAtlas_RemoveCustomRect :: proc(self: ^FontAtlas, id: FontAtlasRectId) ---
	...
	FontAtlas_GetCustomRect :: proc(self: ^FontAtlas, id: FontAtlasRectId, out_r: ^FontAtlasRect) -> bool ---
	...
	FontBaked_FindGlyph :: proc(self: ^FontBaked, c: Wchar) -> ^FontGlyph ---
	// Return NULL if glyph doesn't exist
	@(link_name = "ImFontBaked_FindGlyphNoFallback")
	FontBaked_FindGlyphNoFallback :: proc(self: ^FontBaked, c: Wchar) -> ^FontGlyph ---
	@(link_name = "ImFontBaked_GetCharAdvance")
	FontBaked_GetCharAdvance :: proc(self: ^FontBaked, c: Wchar) -> f32 ---
	@(link_name = "ImFontBaked_IsGlyphLoaded")
	FontBaked_IsGlyphLoaded :: proc(self: ^FontBaked, c: Wchar) -> bool ---
	@(link_name = "ImFont_IsGlyphInFont")
	Font_IsGlyphInFont :: proc(self: ^Font, c: Wchar) -> bool ---
	...
	Font_GetFontBaked :: proc(self: ^Font, font_size: f32, density: f32 = -1.0) -> ^FontBaked ---
	@(link_name = "ImFont_CalcTextSizeA")
	Font_CalcTextSizeA :: proc(self: ^Font, size: f32, max_width: f32, wrap_width: f32, text_begin: cstring, text_end: cstring = nil, out_remaining: ^cstring = nil) -> Vec2 ---
	...
	Font_RenderChar :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vec2, col: u32, c: Wchar, cpu_fine_clip: ^Vec4 = nil) ---
	@(link_name = "ImFont_RenderText")
	Font_RenderText :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vec2, col: u32, clip_rect: Vec4, text_begin: cstring, text_end: cstring, wrap_width: f32 = 0.0, flags: DrawTextFlags = {}) ---
	// [Internal] Don't use!
	...
	Font_AddRemapChar :: proc(self: ^Font, from_codepoint: Wchar, to_codepoint: Wchar) ---
	...
	Viewport_GetDebugName :: proc(self: ^Viewport) -> cstring ---
	...
	PlatformIO_ClearPlatformHandlers :: proc(self: ^PlatformIO) ---
	...
	PlatformIO_ClearRendererHandlers :: proc(self: ^PlatformIO) ---
}
